...
public function validar($classe, $metodo) {
        ...
        if (!$this->usuario_logado() && !in_array($metodo, $ignorar)) {
            $this->redireciona_login();
        } elseif ($this->usuario_logado() && !in_array($metodo, $ignorar)) {
            if (!$this->usuario_tem_permissao($classe, $metodo)) {
                $this->redireciona_sem_permissao();
            }
        }
    }
